...
# window (.rolling / .expanding / .ewm) or a .groupby.
...
_WINDOWERS = frozenset({"rolling", "expanding", "ewm", "groupby", "resample"})
...
def _suppressed_lines(src: str) -> set:
    """Line numbers carrying a `# leak-free:` marker."""
    return {
...
def static_leak_check(
    ...
    functions: Sequence[str] = ("compute_daily_indicators",),
) -> None:
    """
...
def _lookback_for(col: str) -> Optional[int]:
    """Longest window implied by the column name, where one is stated."""
    nums = [int(m) for m in re.findall(r"(\d{1,3})", col)]
    nums = [n for n in nums if n > 1]
    if col.endswith("_52wh") or col.endswith("_52wl") or "52w" in col:
        return 252
...
def feature_contract(df: pd.DataFrame) -> pd.DataFrame:
    ...
    available_at is EOD for everything in this module - each value is a
    function of bars up to and including the current session's close, so the
    earliest a model may act on it is the NEXT session's open. Weekly columns
    are marked as resolving on the prior completed week.
    """
    rows = []
    ...
    for c in df.columns:
        ...
        rows.append({
            "feature": c,
            "available_at": "prior completed week" if weekly else "EOD",
            "lookback_bars": _lookback_for(c),
            "uses_future": c in set(LABEL_COLUMNS) | set(FORWARD_COLUMNS)
                           or _looks_forward(c),
