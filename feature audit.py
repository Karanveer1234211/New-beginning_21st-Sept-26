...
def incremental(df: pd.DataFrame, base: List[str], alive: List[str],
    ...
    Returns (per-fit records, per-fold selection record).
    """
    y = pd.to_numeric(df[target], errors="coerce")
    ...
    return pd.DataFrame(recs), pd.DataFrame(sel_rows)
...
def run(panel_path, *, target: str = DEFAULT_TARGET, mode: str = "marginal",
    ...
    inc, sel = incremental(p, base, alive_c, target, splits, mode=mode,
                           top_candidates=top_candidates, verbose=verbose)
