...
# Path-dependent bracket. TP/SL must match what you actually trade.
LABEL_TP = 0.05
LABEL_SL = 0.03
...
def _build_signature(symbols: Sequence[str], min_turnover: float) -> str:
    ...
    parts = [
        ...
        f"win={LIQ_WINDOW}",
        f"tp={LABEL_TP:.6g}", f"sl={LABEL_SL:.6g}",
        f"h={LABEL_HORIZON}",
...
def compute_labels(
    ...
    first = np.full(n, np.nan)          # 1 = TP first, -1 = SL first, 0 = neither
    d_tp = np.full(n, np.nan)
    d_sl = np.full(n, np.nan)
    ambig = np.zeros(n, dtype="int8")

    for i in range(n):
        j_end = i + horizon
        if j_end >= n or not np.isfinite(c_arr[i]):
            continue
        tp_lvl = c_arr[i] * (1.0 + LABEL_TP)
        sl_lvl = c_arr[i] * (1.0 - LABEL_SL)
        res = 0.0
        for k in range(i + 1, j_end + 1):
            hit_tp = h_arr[k] >= tp_lvl
            hit_sl = l_arr[k] <= sl_lvl
            if hit_tp and hit_sl:
                ambig[i] = 1
                res = -1.0                      # conservative: assume SL first
                d_sl[i] = k - i
                d_tp[i] = k - i
                break
            if hit_tp:
    ...
    out["label_first_touch"] = first
    out["label_tp_before_sl"] = pd.Series(
        np.where(np.isnan(first), np.nan, (first > 0).astype(float)),
        index=out.index).astype("Float64")
    out["label_days_to_tp"] = d_tp
    out["label_days_to_sl"] = d_sl
    out["label_same_day_ambiguous"] = pd.Series(
        np.where(np.isnan(first), np.nan, ambig), index=out.index).astype("Float64")
    return out
