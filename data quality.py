...
class GateResult:
    ...
    @property
    def status(self) -> str:
        return "PASS" if self.allowed else "QUARANTINE"
...
def _check_complete(
    ...
    # Non-NSE calendars (MCX, CDS) cannot be measured against NSE sessions.
    if kind == "futures":
...
def gate_symbol(
    ...
    *,
    ...
    r.complete = _check_complete(df, master, kind, r, absence=absence)
...
def assert_panel_ready(
    ...
    """
    rep = gate_all(root, symbols, reference=reference, schema_version=schema_version)
    if rep["quarantined"]:
        lines = [
