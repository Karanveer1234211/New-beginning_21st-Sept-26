...
LEAKS = {
  ...
  "unannotated groupby":    "df['x'] = df.groupby('k')['v'].rank(pct=True)",
  "unannotated resample":   "df['x'] = s.resample('W').max()",
}
CAUSAL = {
  ...
  "annotated groupby":      "# leak-free: grouped by timestamp, same-date only\n    df['x'] = df.groupby('timestamp')['v'].rank(pct=True)",
...
print("\n" + ("FAILURES: " + "; ".join(bad) if bad else "LEAK CHECKER VERIFIED: 15 caught, 11 clean"))
