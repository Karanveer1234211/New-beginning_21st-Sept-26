...
ORPHAN_WINDOW = 90          # sessions to look ahead
ORPHAN_MIN_DENSITY = 0.60   # share of that window that must be present
ORPHAN_MAX_LEADING = 60     # never trim more than this many leading bars


def find_listing_start(
    ts: pd.Series,
    *,
    window: int = ORPHAN_WINDOW,
    min_density: float = ORPHAN_MIN_DENSITY,
    max_leading: int = ORPHAN_MAX_LEADING,
) -> Optional[int]:
    """
    Index of the first bar of the CONTINUOUS series, or None if it is already
    continuous from the start.

    A bar starts the real history if the next `window` calendar-days-worth of
    sessions are at least `min_density` present. Orphans fail this: 8 bars
    scattered over 2016 have nothing after them for six years.

    max_leading is a safety rail. Trimming hundreds of bars would mean this
    heuristic has misfired on a genuinely thin series, so it refuses instead.
    """
    ...
    n = len(t)
    limit = min(max_leading, n - 1)
    for i in range(limit + 1):
        span_end = t[i] + pd.Timedelta(days=int(window * 1.45))
        in_span = int(((t >= t[i]) & (t <= span_end)).sum())
        expected = np.busday_count(t[i].date(), min(span_end, t[-1]).date())
        if expected <= 5:
            continue
        if in_span / expected >= min_density:
...
def align_to_sessions(
    ...
    *,
    ...
    if series.lag_sessions:
        ...
        fresh = fresh.shift(series.lag_sessions).fillna(False)
