...
# SEARCH
# ----------------------------------------------------------------------
def session_positions(ts: np.ndarray, sessions: np.ndarray) -> np.ndarray:
    """Index of each timestamp within the trading-session calendar."""
    return np.searchsorted(sessions, ts, side="left")


def _decluster(order: np.ndarray, pos: np.ndarray, syms: np.ndarray,
               min_sep_sessions: int) -> np.ndarray:
    """
    Keep the closest analogue in each cluster of near-identical episodes.

    Walking neighbours from nearest outward, an episode is accepted only if
    it is at least `min_sep_sessions` from every already-accepted episode of
    the SAME symbol. Different symbols are independent by construction.

    SEPARATION IS COUNTED IN TRADING SESSIONS, NOT CALENDAR DAYS.
    v3 compared calendar days here while the leakage cutoff already used
    sessions - the same inconsistency, left in one place after being fixed in
    another. Across a weekend, "5 calendar days" is only 3 sessions, so
    episodes whose outcome windows still overlapped were being counted as
    distinct. `pos` is a session index, so the comparison is exact.
    ...
    for i in order:
        s = syms[i]
        p = int(pos[i])
        if any(abs(p - u) < min_sep_sessions for u in last.get(s, [])):
            continue
        kept.append(i)
        last.setdefault(s, []).append(p)
...
def ask(
    ...
    *,
    ...
    near_pos = session_positions(ts[near], sessions)
    kept = _decluster(np.arange(len(near)), near_pos, syms[near], min_separation)
    kept = kept[:k]
    sel = near[kept]
    sel_d = near_d[kept]
