...
r=B.scan(rh,rl,np.array([0.02]),np.array([0.03]))
row=r.loc[(r.tp=="0.05")&(r.sl=="0.030")].iloc[0]
...
r=B.scan(rh,rl,np.array([.0,.0]),np.array([.03,.03]))
row=r.loc[(r.tp=="0.05")&(r.sl=="0.030")].iloc[0]
...
fixed=B.scan(rh,rl,np.zeros(2),atrp,atr=False)
scaled=B.scan(rh,rl,np.zeros(2),atrp,atr=True)
