...
class PanelNotWired(RuntimeError):
    """Stage 3 has no implementation. A run without it is NOT a success."""
...
def run_panel(root: Path, allowed: list[str]) -> None:
    ...
    """
    # v25: this used to print a note and return, so main() returned 0 and the
    # batch file logged "OK" for a run that built no panel. A pipeline that
    # reports success without producing its output is worse than one that
    # fails: you stop checking.
    raise PanelNotWired(
        "stage 3 is not implemented - wire your panel build into run_panel(). "
        "Pass --allow-no-panel to run cache+gate only on purpose."
    )
...
def main() -> int:
    ...
    sys.path.insert(0, str(HERE))
    from Daily_cache_v25 import Config
    ...
    except PanelNotWired as e:
        if not args.allow_no_panel:
            print(f"\n[3/3] PANEL  {e}", file=sys.stderr)
            return 2
